...
class PhotobookEditMetadataRequest(BaseModel): ...


class PhotobooksMetadataResponse(BaseModel): ...
...
class PhotobookAPIHandler(RouteHandler):
    ...
    @enforce_response_model
    async def photobook_edit_metadata(
        self, photobook_id: UUID, payload: PhotobookEditMetadataRequest
    ) -> PhotobooksMetadataResponse: ...
    ...
    async def photobook_edit_comment(
        ...
        async with self.app.new_db_session() as db_session:
            comment = await DALPhotobookComments.get_by_id(
